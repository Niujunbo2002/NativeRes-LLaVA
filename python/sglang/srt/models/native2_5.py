...
def build_vision_projector(config):
    ...
    if mlp_gelu_match:
        ...
        modules = [nn.Linear(config.vision_config.out_hidden_size, config.hidden_size)]
...
class NativeQwen2_5_VLForConditionalGeneration(nn.Module):
    ...
    def get_image_feature(self, items: List[MultimodalDataItem]) -> torch.Tensor:
        ...
        image_embeds = self.mm_projector(image_embeds) # 2x mlp
        return image_embeds
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        for name, loaded_weight in weights:
            ...
            if self.config.tie_word_embeddings and "lm_head.weight" in name:
                continue
            if 'model.vision_tower.vision_tower' in name:
                name = name.replace('model.vision_tower.vision_tower', 'visual')
            if 'model.mm_projector' in name:
                name = name.replace('model.mm_projector', 'mm_projector')
...
EntryClass = [NativeQwen2_5_VLForConditionalGeneration]
